...
- **Shell**: `lib/screens/app_shell.dart` is the post-login container. It holds an `IndexedStack` of the 7 main screens (Dashboard, Attendance, Scan, Billing, Inventory, Chatbot, Settings) and switches between a fixed `Sidebar` (width ≥ 900px) and a `Drawer` + `AppBar` (narrower). `AppShell(initialIndex: N)` is how deep links jump straight to a tab.
